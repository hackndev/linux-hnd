...
static int __init graphicsmaster_init(void)
{
	 ...
	 */
	sa1111_pwm_dutycycle(0, 127);		// VEE
	sa1111_pwm_enable(0);
	sa1111_pwm_dutycycle(1, 1);		// Backlight
	sa1111_pwm_enable(1);
...
static void
gm_irq_handler(unsigned int irq, struct irq_desc *desc, struct pt_regs *regs)
{
	unsigned int mask;
...
static struct irq_chip gm_irq1_chip = {
	.ack	= gm_mask_irq1,
	.mask	= gm_mask_irq1,
...
static struct irq_chip gm_irq2_chip = {
	.ack	= gm_mask_irq2,
	.mask	= gm_mask_irq2,
