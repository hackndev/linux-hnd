...
// T3 workaround #define      DRIVER_NAME     (tps65010_driver.name)
#define DRIVER_NAME "tps65010"
...
u8 tps65010_get_chgstatus(void) {
	if(the_tps)
		return the_tps->chgstatus;
	else
		return 0;
}
EXPORT_SYMBOL(tps65010_get_chgstatus);
...
int tps65010_set_led(unsigned led, unsigned mode)
{
	...
	case UNDER_CHG_CTRL:
		...
		led_per = 0 << 7;
		if(led != LED1) {
			printk(KERN_WARNING "%s: UNDER_CHG_CTRL requested for second LED!\n", DRIVER_NAME);
		}
		break;
	case OFF:
		led_on  = 1 << 7;
