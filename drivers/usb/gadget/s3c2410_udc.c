					   ...
					   unsigned max)
{
	...
	dprintk(DEBUG_VERBOSE, "s3c2410_udc_write_packet %d %d %d ",req->req.actual,req->req.length,len);
	req->req.actual += len;
				  ...
				  struct s3c2410_request *req)
{
	...
	if (is_last) {
		...
		if (!idx) {
			...
			if (! (udc_read(S3C2410_UDC_USB_INT_REG) & S3C2410_UDC_USBINT_RESET))
 ...
 */
static struct usb_request * s3c2410_udc_alloc_request(struct usb_ep *_ep,
						      gfp_t mem_flags)
{
	struct s3c2410_ep	*ep;
	struct s3c2410_request	*req;
 ...
 */
static void s3c2410_udc_free_request(struct usb_ep *_ep,
 ...
 */
static void * s3c2410_udc_alloc_buffer(struct usb_ep *_ep, unsigned bytes,
			     ...
			     gfp_t gfp_flags)
{
	...
	if (unlikely (!_ep || (!ep->desc && ep->ep.name != ep0name))) {
		dprintk(DEBUG_NORMAL, "s3c2410_udc_queue: inval 2\n");
		return -EINVAL;
	}
	...
	if (unlikely (!_req || !_req->complete || !_req->buf ||
		...
		if (!_req)
			dprintk(DEBUG_NORMAL, "s3c2410_udc_queue: 1 X X X\n");
		else {
			dprintk(DEBUG_NORMAL, "s3c2410_udc_queue: 0 %01d %01d %01d\n",!_req->complete,!_req->buf, !list_empty(&req->queue));
		}
	...
	dprintk(DEBUG_VERBOSE, "s3c2410_udc_queue: ep%x len %d\n",
	...
	if (list_empty(&ep->queue) && !ep->halted) {
		if (ep->bEndpointAddress == 0 /* ep0 */) {
			...
			req = NULL;
			   ...
			   fifo_count && s3c2410_udc_read_fifo(ep, req)) {
			req = NULL;
		}
	}
	...
	dprintk(DEBUG_VERBOSE, "s3c2410_udc_queue normal end\n");
	return 0;
}
...
static int s3c2410_udc_set_halt(struct usb_ep *_ep, int value)
{
	...
	ep = container_of(_ep, struct s3c2410_ep, ep);
	if (unlikely (!_ep || (!ep->desc && ep->ep.name != ep0name))) {
		dprintk(DEBUG_NORMAL, "s3c2410_udc_set_halt: inval 2\n");
		return -EINVAL;
	}
 ...
 */
static int s3c2410_udc_set_selfpowered(struct usb_gadget *_gadget, int value)
{
	struct s3c2410_udc  *udc;
...
static int s3c2410_udc_pullup(struct usb_gadget *_gadget, int is_on)
{
	struct s3c2410_udc  *udc;
...
static void s3c2410_udc_disable(struct s3c2410_udc *dev)
{
	dprintk(DEBUG_NORMAL, "s3c2410_udc_disable called\n");
...
static void s3c2410_udc_enable(struct s3c2410_udc *dev)
{
	...
	udc_write(1<<2 | 1<<0 ,S3C2410_UDC_USB_INT_EN_REG);

	/* Enable ep0 interrupt */
	udc_write(0x01,S3C2410_UDC_EP_INT_EN_REG);
...
static int s3c2410_udc_probe(struct platform_device *pdev)
{
	...
	if (s3c2410_udc_debugfs_root) {
		udc->regs_info = debugfs_create_file("registers", S_IRUGO,
			s3c2410_udc_debugfs_root, udc, &s3c2410_udc_debugfs_fops);
...
static int __init udc_init(void)
{
	...
	if (IS_ERR(s3c2410_udc_debugfs_root)) {
		printk(KERN_ERR "%s: debugfs dir creation failed %lx\n",
