...
int sa1100_audio_suspend(audio_state_t *s, pm_message_t state)
{
...
int sa1100_audio_resume(audio_state_t *s)
{
	audio_stream_t *is = s->input_stream;
	audio_stream_t *os = s->output_stream;
			...
			audio_state_t *state)
{
	...
	seq1=register_sound_special(&dev_fileops, 1);
	if(seq1==-1)
		goto bad;
	seq2=register_sound_special(&dev_fileops, 8);
	if(seq2!=-1)
		return 0;
